...
j = sqrt(-1);
datalength_train = 60000; %change to 60000

Np = 1000;     %CHANGE TO 20                    %number of parfors
Nd = datalength_train / Np;      %number of examples per parfor

SNRdB = -3 ; %in db 
varNoise = 1;   %variance of noise 

Nt = N+1;           %number of columns of H = [d B],H is the channel marix

pl_f = 0.18^-2 / 31.6;  %these are the path loss  modeelling functions
pl_G = 1/31.6;
pl_d = 1/33;

K_dB = 10 ;  %Rician k factor in db
K = 10^(K_dB/10) ;  %Rician k factor in linear scale 

Wn = exp ( j* 2*pi / C ) ; %complex factor used for phase shift 
temp = 0:C-1;
s = [];  %phase shift matrix for IRS reflection patterns 
...
for tt = 1 : Np  %this is used to combine the data
